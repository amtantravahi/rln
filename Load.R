...
lexis_corpus_process <- function(data, text_var) {
  ...
  english_stopwords <- getURL("https://raw.githubusercontent.com/stopwords-iso/stopwords-en/master/stopwords-en.txt",  encoding = "UTF-8")
  english_stopwords <- read.csv(text = english_stopwords, header = F)
  english_stopwords <- english_stopwords$V1
  
  spanish_stopwords <- getURL("https://raw.githubusercontent.com/stopwords-iso/stopwords-es/master/stopwords-es.txt", encoding = "UTF-8")
  spanish_stopwords <- read.csv(text = spanish_stopwords, skip = 11, header = F)
  spanish_stopwords <- spanish_stopwords$V1
  
  french_stopwords <- getURL("https://raw.githubusercontent.com/stopwords-iso/stopwords-fr/master/stopwords-fr.txt", encoding = "UTF-8")
  french_stopwords <- read.csv(text = french_stopwords, header = F)
  french_stopwords <- french_stopwords$V1
  
  german_stopwords <- getURL("https://raw.githubusercontent.com/stopwords-iso/stopwords-de/master/stopwords-de.txt", encoding = "UTF-8")
  ...
  processedCorpus <- tm_map(processedCorpus, removeWords, spanish_stopwords)
  processedCorpus <- tm_map(processedCorpus, removeWords, french_stopwords)
  ...
  processedCorpus <- tm_map(processedCorpus, stemDocument, language = "fr")
